...
Welcome to the landing page for my Maths PSMT, you can find the link to the GitHub repository here and also the full explanation found in the appendix below because I wanted to save some trees.

## Explanation of Program

As mentioned before the model is graphically created in the physics engine Unity. This also means that the calculations for this model could also be conducted within Unity, via a script written by me in the programming language C#, allowing for very high precision. The program has 3 major parts, initial variables, the Start function and the explicitly named Maths functions.
Maths Functions

### Maths Functions

Let’s start with the Maths function which calculates the surface area of a rectangular prism with the ability to dynamically add or remove sides from the prism. This function named RectprismSA will return a float, which is a high precision data type storing a floating-point number.
...
This will eventually be returned as required by the function. Next a conditional called an if-else statement checks the two dimensional array’s first pair for a string called wl from the first dimension.
...
After that the second dimension’s contents is “parsed” (In this context parsing is the process of turning a string into a float). This value is then applied to the newly named float variable wlAmount. Thus making wlAmount equal to the parsed value.
...
Finally the runningtotal is returned by the function. Ending and destroying this function.
...
The next function that is used is CylinderSA, which calculates the surface area of a cylinder with the ability to remove the two circles found at each end of a cylinder. Before we name this function we have to create an enumeration which contains a list referred to as the enumerator list. This list contains multiple explicitly named constants. In this program I used an enumeration called CylinderCircleCount with the enumerator list which denotes the possible number of circles found on a cylinder, either zero, one or two.
...
Initial variables are name at the top below the class declaration. Variables allocate system Random Access Memory (RAM or Memory) ready for data to be stored in this allocated space. Variables need a datatype and an identifier or name. In the model that I have created every rectangular prism requires a two dimensional string (a string is an array of unicode characters) array to be initialised, these are initialised with the variables. Unity’s proprietary datatype GameObject represents a lot of data including a GameObject’s dimensions in physical (virtual) space. We need this in order to dynamically calculate surface area and volume on the fly. This is why every component that is being utilised in my model has a variable of the datatype GameObject. Below are these initial variables and accompanying two dimensional arrays.
...
The Start function is the entry point for the compiler, similar to the Main function in a terminal or command prompt project. When the program is run it will only execute anything found in the Start function (and also the Update function but that is irrelevant). Nothing else in the program will execute by itself. To utilise functions such as RectprismSA or CylinderSA they first need to be called from the Start function. In the Start function there are two major blocks of code, one calculates the total surface area then prints it to Unity’s debug log.
